...
func (b *base) ReloadCert() error {
	...
	if b.cfg.CertPath == "" {
		return trace.Errorf("Attempted to reload cert for a gateway, but CertPath is empty")
	}

	if b.cfg.KeyPath == "" {
		return trace.Errorf("Attempted to reload cert for a gateway, but KeyPath is empty")
	}
