...
func onSSHLatency(cf *CLIConf) error {
	...
	nodeClient, err := tc.ConnectToNode(
		cf.Context,
		clt,
		client.NodeDetails{Addr: tc.Host + ":0", Namespace: tc.Namespace, Cluster: tc.SiteName},
		tc.Config.HostLogin,
	)
	if err != nil {
		tc.ExitStatus = 1
	...
	clientPinger, err := latency.NewSSHPinger(nodeClient.Client)
	if err != nil {
		return trace.Wrap(err)
	}

	return trace.Wrap(showLatency(cf.Context, clientPinger, clt.ProxyClient, tc.Host, "Proxy"))
}
