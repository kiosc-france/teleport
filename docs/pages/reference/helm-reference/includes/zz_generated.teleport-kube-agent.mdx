...
`roles` is a comma-separated list of services which should be enabled
...
For example: `roles: kube,app,discovery`.
...
instance join the Teleport cluster.

See [the join method reference](../join-methods.mdx) for the list of possible
values, the implications of each join method, and guides to setup each method.

Common join-methods for the `teleport-kube-agent` are:
- `token`: the most basic one, with relgular ephemeral secret tokens
...
<Admonition type="tip" title="Supported values">
  You can see a list of all the supported[values which can be used in a Teleport
  application access configuration here](../../application-access/reference.mdx#configuration).
</Admonition>
...
`tsh` by following [this guide](../../application-access/guides/dynamic-registration.mdx).
</Admonition>
...
`kubernetesDiscovery` controls configuration for discovery service
if it's enabled.
...
documentation](../../application-access/enroll-kubernetes-applications/architecture/)
for more details.

The default value will try to discovery all apps running in Kubernetes.
...
| Type | Default |
|------|---------|
| `string` | `"https://updates.releases.teleport.dev/v1/"` |
...
`existingDataVolume` is the name of an existing Kubernetes Peristent
...
The availability can be increased by running more replicas with `replicaCount`,
requiring that the Pods are not scheduled on the same Kubernetes Node with
`requireAntiAffinity`, and by asking Kubernetes not to delete all pods at the
same time with `podDisruptionBudget`.
...
This setting only takes effect when [`enterprise`](#enterprise-1) is `false`.
...
`image` sets the container image used for Teleport Enterprise agent
...
This setting only takes effect when [`enterprise`](#enterprise-1) is `true`.
...
`initContainers` sets the Teleport Pod's init-conatiners.
